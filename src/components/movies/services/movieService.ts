...
let rqst_TMDB = require("../../../utils/request");
let { favoritemovieModel, movieModel } = require("../../../models/mongo");
...
class Movie {
  ...
  async getFavorites(user:IUser){
    try {
      return await favoritemovieModel.find({userId: ObjectId(user._id)}, {_id:0, userId:0, __v:0}).populate({
        path: 'movieId',
        model: 'movie'
      });
    } catch (error:any) {
